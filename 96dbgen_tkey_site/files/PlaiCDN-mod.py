...
def report_chunk(bytes_so_far, chunk_size, total_size):
    ...
    sys.stdout.write('\rDownloaded and decrypted %d (%d mb) of %d bytes (%0.2f%%)' % (bytes_so_far, bytes_so_far / 1000000, total_size, percent))
    sys.stdout.flush()
...
def read_chunk(response, f_out, intitle_key, first_iv, chunk_size=0x200000, report_hook=None):
    ...
    while 1:
        ...
        if not chunk or bytes_so_far >= 30000000:
             break
